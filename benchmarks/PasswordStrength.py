...
def check_password_strength2(password):
    ...
    for char in list(password):
        if char in string.ascii_lowercase:
            ...
            special_char_count = 1 # <-- FIX special_char_count += 1
    strength = 0
    remarks = ''
...
def check_password_strength3(password):
    lower_alpha_count = upper_alpha_count = number_count = whitespace_count = special_char_count = 0
    for char in list(password):
        if char in string.ascii_lowercase:
            lower_alpha_count += 1
        elif char in string.ascii_uppercase:
            upper_alpha_count = 1 # <-- FIX upper_alpha_count += 1
        elif char in string.digits:
            number_count = 1 # <-- FIX number_count += 1
        elif char == ' ':
            whitespace_count = 1 # <-- FIX whitespace_count += 1
        else:
            special_char_count += 1
    ...
    if upper_alpha_count >= 1:
        strength += 1
    if number_count >= 1:
        strength = 1 # <-- FIX strength += 1
    if whitespace_count >= 1:
        strength += 1
    if special_char_count == 1: # <-- FIX if special_char_count >= 1:
        strength += 1
